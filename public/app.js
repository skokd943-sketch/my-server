...
// Уникальный цвет аватарки для каждого имени (хэш строки -> оттенок)
...
async function showChat() {
  ...
  setAvatar(document.getElementById('meAvatar'), myName);
  await loadContacts();
  connectWS();
}
...
async function loadContacts() {
  try {
    ...
    users.forEach(u => {
      ...
      div.dataset.user = u;
      div.innerHTML = `<span class="avatar" style="${avatarStyle(u)}" data-initial="${initial(u)}"></span><span class="contact-name">${u}</span><span class="dot" data-dot="${u}"></span>`;
...
function renderMessage(m) {
  ...
  if (m.kind === 'image') {
    ...
    bubble.innerHTML = `<a class="file-chip" href="${m.fileUrl}" target="_blank" download><span class="file-icon">📄</span><span>${name}</span></a>`;
  } else {
